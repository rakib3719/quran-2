...
const juzSummary = Array.from({ length: 30 }, (_, i) => {
  const juz = i + 1;
  const inJuz = surahs.filter((surah) => surah.ayahs.some((ayah) => ayah.juz === juz));
  return {
    juz,
...
export default function SidebarRight({ mode, selectedId, onModeChange }: Props) {
  ...
  const filteredSurahs = useMemo(() => {
    ...
    return surahs.filter((surah) =>
      [surah.englishName, surah.englishNameTranslation, surah.number.toString(), surah.name]
        .join(" ")
        .toLowerCase()
        .includes(term)
    );
  ...
  const filteredJuz = useMemo(() => {
    ...
    return juzSummary.filter((item) => `juz ${item.juz}`.includes(term));
  }, [query]);
          ...
          {(["surah", "juz", "page"] as ReaderMode[]).map((item) => (
            <button
              ...
              className={`h-10 rounded-full capitalize transition-colors ${
                mode === item ? "bg-[#090b10] text-white" : "text-[#8a9099]"
              }`}
            >
              {item}
            </button>
        ...
        <div className="mt-4 space-y-3">
            ...
            filteredSurahs.map((surah) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-lg font-semibold">{surah.englishName.replace("-", " ")}</p>
                    <p className="text-[#7d8490]">{surah.englishNameTranslation}</p>
                  </div>
                  <p className="text-[#89909c] text-lg">{surah.number}</p>
                </div>
              </button>
            ))}
          ...
          {mode === "juz" &&
            filteredJuz.map((item) => (
              ...
              >
                <p className="text-[#42a34f] text-xl font-semibold">Juz {item.juz}</p>
                <p className="text-[#7d8490]">{item.first?.englishName ?? "Surah"} & More</p>
                <p className="text-[#9aa0a9] mt-1">{item.surahCount} Surah</p>
              </button>
            ))}
          ...
          {mode === "page" &&
            filteredPages.map((pageNumber) => (
              ...
              >
                <p className="text-xl font-semibold">Page {String(pageNumber).padStart(2, "0")}</p>
              </button>
            ))}
        </div>
      </div>
